...
## 场景零：同一会话中自由切换智能体与模型（控制 Token 费用）

**目标**：在**同一通道或桌面对话**中自由切换机器人/智能体，并为单个智能体自由切换不同大模型，便于**限制 Token 费用**、在单一通道快速使用所有类型机器人。

**适用**：桌面端、Web 端、飞书/钉钉/Telegram/微信等通道。

### 同一会话中自由切换智能体

1. **不新建会话**：在任意一个会话（桌面对话或通道对话）中，无需新建会话即可切换当前使用的智能体。
2. **使用 `//` 指令**：输入 `//select` 查看当前可用智能体列表；输入 `//智能体名` 或 `//id` 切换到指定智能体（本机智能体或代理智能体如 Coze、OpenClawX、OpenCode、Claude Code）。
3. **界面选择**：在桌面/Web 对话页上方的智能体选择器中直接点击要使用的智能体即可切换。
4. **效果**：可在**同一通道**里快速使用所有类型机器人——例如先让「编码助手」写代码，再输入 `//周报助手` 切到周报助手写周报，无需为每个机器人单独开会话。

### 单智能体自由切换不同大模型

1. **进入配置**：打开 **设置 → 智能体**，选择要改模型的智能体并进入其详情；或打开 **设置 → 模型配置**，在「模型配置」中为某 Provider 选择/更换模型。
2. **为智能体选择模型**：在该智能体的「模型配置」中选择不同的 Provider + 模型（如从「本地 Qwen3.5 4B」改为「DeepSeek」或「Ollama qwen3:8b」），保存即可。
3. **控制 Token 费用**：日常或简单任务可为该智能体选用**本地模型/小模型**（0 或低 Token）；复杂任务时再为同一智能体切换到**云端大模型**，便于按预算与任务灵活控制费用。

**提示**：保存智能体配置后，该智能体对应的运行会话会自动刷新，下次对话即使用新配置；详见 [桌面端使用 - 同一会话中自由切换智能体与模型](desktop-usage.md#同一会话中自由切换智能体与模型)。
