...
在 OpenClawX 中，**同一会话**（桌面对话或通道对话）内可以**自由切换智能体**，**单个智能体**也可以**自由切换不同大模型**，便于在通道中快速使用所有类型机器人，并**控制 Token 费用**。

### 同一会话中自由切换智能体

- **含义**：在**不新建会话**的前提下，在当前对话里随时切换为任意已配置的智能体（本机智能体或代理智能体，如 Coze、OpenClawX、OpenCode、Claude Code）。
- **价值**：在飞书、钉钉、Telegram、微信等**同一通道**中，可快速切换不同「机器人」完成不同任务，无需为每个机器人单独开会话。
- **方式**：使用下方「对话内 `//` 指令」或桌面/Web 界面中的智能体选择器切换。

### 单智能体自由切换不同大模型

- **含义**：每个智能体在「模型配置」或智能体详情中可**独立选择并更换**所用大模型（如 DeepSeek、Qwen、Ollama、本地 GGUF 等）。
- **价值**：便于**控制 Token 费用**——日常或简单任务选用本地模型/小模型，复杂任务再为该智能体切换到云端大模型；或按预算在多个模型间灵活切换。
...
桌面端本地向量**仅使用 node-llama-cpp (GGUF)**。默认使用 `embeddinggemma-300M-Q8_0.gguf`（约 0.6 GB），首次运行会按 node-llama-cpp 规则下载到 `~/.openbot/.cached_models/`。可在桌面配置中设置 `rag.localModelPath`（如本地 GGUF 路径或 `hf:...` URI）覆盖默认模型。若加载失败并出现 `Unexpected token 'with'`，多为依赖链中某包在 ESM 下使用了禁止的 `with` 语句，可升级 Electron 至 Node 22+ 或**在设置中配置 RAG 在线模型**。
