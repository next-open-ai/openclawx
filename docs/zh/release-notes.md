...
### 同一会话自由切换智能体与单智能体自由切换模型

- **同一 Session 中自由切换智能体**
  - 在**同一会话**（桌面对话或通道对话）中可随时切换当前使用的智能体，包括**本机智能体**与**代理智能体**（Coze / OpenClawX / OpenCode / Claude Code），无需新建会话。
  - 通过对话内 `//` 指令（如 `//select` 查看列表、`//智能体名` 或 `//id` 切换）或界面选择即可完成切换，便于在**单一通道中快速使用所有类型机器人**。
- **单智能体自由切换不同大模型**
  - 每个智能体可独立配置并随时更换所用**大模型**（Provider + 模型）；在「模型配置」或智能体详情中为同一智能体选择不同模型即可。
  - 便于按任务难度或预算**控制 Token 费用**：日常用本地/小模型，复杂任务再切到云端大模型。

### 本地推理与模型推荐

- **本地 node-llama-cpp 推理**：本机执行（local）+ 本地 GGUF 时，基于 **node-llama-cpp** 子进程，**全量仿真 OpenAI 协议**，提供**类本地 OpenAI 服务**，兼容标准 OpenAI 客户端与接口。
- **模型对比与推荐**：目前国内开源表现较好的模型为 **Qwen3.5** 系列；推荐在「设置 → 模型配置 → 本地模型」中下载 Qwen3.5 4B/8B 等用于本地 0 Token 推理。

### 其他

- 配置更新后运行中 AgentSession 即时失效，下次对话自动使用新配置；保存智能体配置时前端提示「该智能体对应的会话已刷新，下次对话将使用新配置」。
- Gateway 端口（默认 38080）被占用时自动尝试下一端口（38081、38082…），桌面端与前端同步使用实际端口，避免启动失败。
